...
import { showCustomToast, Loader, FailSafePage } from "@ucc/common-ui";
...
const GroupHierarchyPage = () => {
    ...
    const fetchHierarchy = async () => {
      ...
      try {
        const response: HierarchyPageData = await api.get(
          `${API_ENDPOINTS.groups}/${id}${API_ENDPOINTS.hierarchy}`
        );
  ...
  return (
    <CustomCards title="" className="card-container-hierarchy">
      <OrgTree data={data} data-testId="org-tree" currentGroupId={id} />
    </CustomCards>
  );
};
